...
def decode_subst_char(msg, steps=4000, restarts=90): 
    "Decode a substitution cipher with random restart hillclimbing. CLM version" 
    candidates = [hillclimb(encode(msg, key=cat(shuffled(alphabet))), 
                            logP3letters, neighboring_msgs, steps) 
                  for _ in range(restarts)] 

    p, decipherment = max((logP3letters(c), c) for c in candidates) 
    return decipherment 
